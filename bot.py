...
# Railway dashboard da qo'shing:
#   BOT_TOKEN    = sizning bot tokeningiz
#   ADMIN_ID     = sizning Telegram ID ingiz
#   CHANNEL_USERNAME = @kanal_nomi  (ixtiyoriy)

BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
if not BOT_TOKEN:
    raise ValueError("BOT_TOKEN environment variable topilmadi! Railway -> Variables ga qo'shing.")

ADMIN_ID = int(os.environ.get("ADMIN_ID", "0"))
if ADMIN_ID == 0:
    raise ValueError("ADMIN_ID environment variable topilmadi! Railway -> Variables ga qo'shing.")

CHANNEL_USERNAME = os.environ.get("CHANNEL_USERNAME", "@MatematikaQuizchi")
...
# Railway da persistent disk uchun /data/ ishlatiladi
# Railway -> Settings -> Add Volume -> Mount path: /data
# Agar volume ulashmasangiz, har restart da DB o'chadi!
DB_PATH = os.environ.get("DB_PATH", "/data/quiz_bot.db")
...
async def update_group_quiz_stats(user_id, group_id, score):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            'INSERT OR IGNORE INTO group_quiz_stats (user_id, group_id) VALUES (?, ?)',
            (user_id, group_id)
        )
...
async def get_referral_stats(user_id):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            'INSERT OR IGNORE INTO referrals (user_id, referrer_id) VALUES (?, NULL)',
            (user_id,)
        )
        await db.commit()
...
async def get_profile_settings(user_id):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            'INSERT OR IGNORE INTO profile_settings (user_id) VALUES (?)',
            (user_id,)
        )
        await db.commit()
...
async def get_reminder_settings(user_id):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            'INSERT OR IGNORE INTO user_reminders (user_id) VALUES (?)',
            (user_id,)
        )
        await db.commit()
        cursor = await db.execute(
...
async def get_speed_quiz_stats(user_id):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            'INSERT OR IGNORE INTO speed_quiz_stats (user_id) VALUES (?)',
            (user_id,)
        )
        await db.commit()
...
async def update_season_stats(user_id, username, correct, wrong):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Foydalanuvchini qo'shish (agar yo'q bo'lsa)
        await db.execute(
            'INSERT OR IGNORE INTO season_stats (user_id, username) VALUES (?, ?)',
            (user_id, username)
        )
...
async def get_pvp_stats(user_id):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        # Foydalanuvchini qo'shish
        await db.execute(
            'INSERT OR IGNORE INTO pvp_stats (user_id) VALUES (?)',
            (user_id,)
        )
...
# ==================== RAILWAY PATCH ====================
# main() funksiyasini Railway uchun moslaymiz
# (yuqoridagi main() o'rnini bosadi)
...
def main():
    ...
    # Railway uchun xatolikda qayta ulanish
